...
void NCursesDisplay::DisplaySystem(System& system, WINDOW* window) {
  ...
  row = 0;
  int const lbl_column = window->_maxx - 70;
  int const bar_column = window->_maxx - 62;
  mvwprintw(window, ++row, lbl_column, "CPU: ");
  wattron(window, COLOR_PAIR(1));
  mvwprintw(window, row, bar_column, "");
  wprintw(window, ProgressBar(system.Cpu().Utilization("cpu")).c_str());
  wattroff(window, COLOR_PAIR(1));

  mvwprintw(window, ++row, lbl_column, "CPU1: ");
  wattron(window, COLOR_PAIR(1));
  mvwprintw(window, row, bar_column, "");
  wprintw(window, ProgressBar(system.Cpu().Utilization("cpu0")).c_str());
  wattroff(window, COLOR_PAIR(1));
  
  mvwprintw(window, ++row, lbl_column, "CPU2: ");
  wattron(window, COLOR_PAIR(1));
  mvwprintw(window, row, bar_column, "");
  ...
  wrefresh(window);
  ++row;
  mvwprintw(window, ++row, lbl_column, "Memory: ");
  wattron(window, COLOR_PAIR(1));
  mvwprintw(window, row, bar_column, "");
...
void NCursesDisplay::DisplayProcesses(std::vector<Process>& processes,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    mvwprintw(window, row, stat_column, processes[i].Status().c_str());
    mvwprintw(window, row, ram_column, processes[i].Ram().c_str());
    mvwprintw(window, row, time_column,
              Format::ElapsedTime(processes[i].UpTime()).c_str());
    mvwprintw(window, row, command_column,
              processes[i].Command().substr(0, window->_maxx - 48).c_str());
  }
}
